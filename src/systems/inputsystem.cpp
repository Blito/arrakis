...
void InputSystem::notify(core::MessageReceiver::Message msg)
{
    ...
    auto change_action_status =
            [&limit_chars, this] (const std::string & action_tag, const core::MessageReceiver::Message & msg, bool new_status)
            {
                auto action_begin_pos = msg.find(action_tag) + action_tag.size();
